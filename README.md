...
| `glm-4.7-flash` | Сбалансированная, рекомендуемая |
| `glm-4.5-air` | Быстрая, менее точная |

### Параметры генерации

Приложение поддерживает расширенные параметры управления генерацией текста:

#### Основные параметры

| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `do_sample` | Boolean | `true` | Использовать ли семплирование для разнообразия ответов |
| `temperature` | Float | модельная | Контролирует случайность (0.2 = консервативно, 0.8 = креативно) |
| `top_p` | Float | модельная | Nucleus sampling (0.1-0.3 = консервативно, 0.8-0.95 = разнообразно) |
| `max_tokens` | Int | модельная | Максимальное количество токенов в ответе |
| `stream` | Boolean | `false` | Потоковый режим (SSE) для отображения ответа в реальном времени |
| `thinking` | Object | `enabled` | Режим цепочки рассуждений для сложных задач |

#### Предустановки параметров

Для удобства предусмотрены готовые конфигурации:

- **Creative** — для творческих задач (высокая креативность, `temperature = 0.8`)
- **Technical** — для технических задач (детерминированность, `temperature = 0.2`)
- **Balanced** — сбалансированная (подходит для большинства задач, `temperature = 0.5`)
- **Fast** — быстрая (максимальная скорость, короткие ответы)
...
📖 **Подробнее:** См. [API_PARAMETERS.md](API_PARAMETERS.md) для полной документации по параметрам.
